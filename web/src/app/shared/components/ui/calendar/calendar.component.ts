...
import type {
	CalendarMode,
	CalendarValue,
} from '@/shared/components/ui/calendar/calendar.types'
import {
...
} from '@/shared/components/ui/calendar/calendar.utils'
import { calendarVariants } from '@/shared/components/ui/calendar/calendar.variants'
import { ZardCalendarGridComponent } from '@/shared/components/ui/calendar/calendar-grid.component'
import { ZardCalendarNavigationComponent } from '@/shared/components/ui/calendar/calendar-navigation.component'
...
export class ZardCalendarComponent implements ControlValueAccessor {
	...
	protected readonly calendarDays = computed(() => {
		...
		const navigationDate = makeSafeDate(
			Number.parseInt(this.currentYearValue(), 10),
			Number.parseInt(this.currentMonthValue(), 10),
			currentDate.getDate(),
		)
	...
	protected previousMonth(): void {
		const month = Number.parseInt(this.currentMonthValue(), 10)
		const year = Number.parseInt(this.currentYearValue(), 10)
